...
async function runRenderCritic(
  ...
  try {
    // A schema miss gets one fresh attempt before the honest skip: the model
    // occasionally returns the wrong verdict count or a stray enum, and one
    // retry recovers most of those without changing the rule that a result
    // which cannot be validated is never shown.
    let output;
    let errors;
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      ...
      errors = validateVerdicts(output, checks);
