...
//
// Only the Intake Agent currently talks to a model provider; the rest are
// deterministic generators over the structured brief. They share Intake's
// response envelope so swapping a provider in later is a per-agent change.
...
async function runTask(taskId, ctx, { provider, intakeHeuristics, production }) {
  ...
  if (taskId === "shots") {
    ctx.shotList = production.buildShotList(ctx.structuredBrief, ctx.enforce);
    ...
    return envelope({
      ...
      generatedBy: "derived",
      auditMessage: `Shot Agent generated ${ctx.shotList.shots.length} timed shots for ${ctx.shotList.duration_seconds}s.`,
    });
  }
