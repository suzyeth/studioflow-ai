    ...
    async generateJson({ system, user }) {
      ...
      for (let attempt = 1; ; attempt += 1) {
        try {
          ...
          // The SDK throws ApiError carrying `.status`, but its message is the
          // raw response body with no status line. Normalise to the same
          // `HTTP <code>` shape the fetch version produced: runIntakeAgent
          // surfaces this string as degraded_reason, and the audit trail reads
          // better with the code — and, when retries were spent, with the count.
          if (status !== null) {
